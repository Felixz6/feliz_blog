...
for (let i = 0; i < data.length; i += 4) {
  ...
  for (let channel = 0; channel < 3; channel++) {
    data[i + channel] = Math.round(data[i + channel] + (255 - data[i + channel]) * 0.1 * weight);
  }
}
...
const target = new URL("../public/themes/kisara/assets/blog/kisara-front-blade-v3.webp", import.meta.url);
